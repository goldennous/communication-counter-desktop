...
export async function getCounterByDate(
  date: string
  ...
  return (result.Item as CounterEntry) ?? null;
}

export async function upsertCounter(
  date: string,
  calls: number,
  chats: number
...
export async function deleteCounterByDate(date: string): Promise<void> {
  await docClient.send(
    new DeleteCommand({
      TableName: TableName.COUNTERS,
      Key: { date },
...
export async function getCountersByDateRange(
  startDate: string,
  endDate: string
): Promise<CounterEntry[]> {
  const result = await docClient.send(
  ...
  return (result.Items as CounterEntry[]) ?? [];
}
